...
methods: {
  updateSearchResults() {
    this.searchgoodlist = this.goodlist.filter(item => {
      const combinedName = item.char1 + item.char2;
      const meetsKeywordCriteria = combinedName.includes(this.$route.query.keyword);
      const meetsPriceCriteria = (this.price1 === null || item.sell >= this.price1) && (this.price2 === null || item.sell < this.price2);
      return meetsKeywordCriteria && meetsPriceCriteria;
    });

    this.searchgoodlist.sort((a, b) => a.sell - b.sell);
  }
}
}
</script>
...
<template>
    <div class="price">
        <label class="text">价格区间</label>
        <input type="number" v-model="price1" placeholder="￥">
        <label>-</label>
        <input type="number" v-model="price2" placeholder="￥">
      </div>
    <!-- <div>
        <p>关键词：{{ searchgoodlist }}</p>
    </div> -->
  <div class="title">
    <h1>所有宝贝</h1>
  </div>
  <div class="pic" v-infinite-scroll="load">
    <li v-for="(item,index) in searchgoodlist.slice(0, count)" :key="item.index" class="infinite-list-item">
        <RouterLink :to="`/detail/${item.id}`">
        <div class="container">
            <div class="left">
                <img :src="item.imageUrl1" alt="Left Image" class="image">
            </div>
            <div class="right">
                <span><img :src="item.imageUrl2" alt="" class="im1"></span>
                <div class="character"><p>{{item.char1}}</p><p>{{item.char2}}</p></div> 
                <div class="sell">￥<span>{{item.sell}}</span></div> 
            </div>
           </div>
        </RouterLink>
        </li>
...
.pic li .container .right  {
    padding:15px 20px 10px 5px; 
}
.pic li .container .right .im1 {
    padding:5px 16px 0px 0px; 
    float:left;
    height:20px;
    width: 60px;
}
.pic li .container .right .character {

    font-family:"宋体";
    font-size: 18px;
    line-height: 30px;
}
.pic li .container .right .sell {
    padding-top:40px;
    color:#9a2e1f;
    font-size: 20px;

}
.pic li .container .left .image {
    background-color: #f5f5f5;
    float: left;
    width: 150px;
    height:150px;
